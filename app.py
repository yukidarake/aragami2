...
def ip_banned():
    return config['ip_ban_threshold'] <= count_failure_by_ip(request.remote_addr)

def count_failure_by_ip(remote_addr):
    global config
    cur = get_db().cursor()
    cur.execute(
...
def initialize():
    cur = get_db().cursor()
    cur.execute('SELECT ip FROM login_log GROUP BY ip')
    rows = cur.fetchall()
    for row in rows:
        ip = row['ip']
